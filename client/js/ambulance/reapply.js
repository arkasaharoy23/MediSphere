...
async function prefillForm() {
  ...
  form.vehicleNumber.value = profile.vehicleNumber || '';
...
function initForm() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const result = await authFetch('/api/ambulance/resubmit', {
      method: 'PUT',
      body: formData
    });
...
document.addEventListener('DOMContentLoaded', () => {
  initDashboard({
    ...
    onReady: (session) => {
      ...
      initFileField('rcDocument', '[data-rc-label]', '[data-rc-text]');
      initFileField('driverLicenseDoc', '[data-license-label]', '[data-license-text]');
      initFileField('driverPhoto', '[data-photo-label]', '[data-photo-text]');
      initFileField('driverIdDoc', '[data-id-label]', '[data-id-text]');
      initFileField('permitDocument', '[data-permit-label]', '[data-permit-text]');
      initLocationDetect();
      initForm();
      prefillForm();
    }
