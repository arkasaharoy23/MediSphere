...
async function prefillForm() {
  // Hospital has no GET /profile endpoint (unlike Doctor/Lab/Pharmacy) — there's
  // nothing to prefill from, so the applicant re-enters everything fresh here,
  // same as they did at original registration.
}
...
function initForm() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const result = await authFetch('/api/hospital/resubmit', {
      method: 'PUT',
      body: formData
    });
...
document.addEventListener('DOMContentLoaded', () => {
  initDashboard({
    ...
    onReady: (session) => {
      ...
      initFileField('document1', '[data-doc1-label]', '[data-doc1-text]');
      initFileField('document2', '[data-doc2-label]', '[data-doc2-text]');
      initLocationDetect();
      initForm();
      prefillForm();
    }
